 ...
 *   lpcOrder: 46  // Optional: defaults to sampleRate/1000 + 2 (46 for 44100 Hz)
 * });
 *
...
export async function extractFormants(
  ...
  // Formula: lpcOrder = sampleRate / 1000 + 2 (standard for formant analysis)
  // Higher sample rates need higher LPC order to capture F2/F3
  const MIN_LPC_ORDER = 8;
  const MAX_LPC_ORDER = 50; // Allow high values for high sample rates
  const defaultLpcOrder = Math.floor(sampleRate / 1000) + 2;

  const lpcOrder = Math.max(
    MIN_LPC_ORDER,
    Math.min(MAX_LPC_ORDER, options?.lpcOrder ?? defaultLpcOrder)
  );
